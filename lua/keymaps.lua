...
map(
  ...
  '<DOWN>',
  "v:count == 0 ? 'gj' : 'j'",
  { desc = 'Down ignore wrapped', expr = true, silent = true }
)
map(
  { 'n', 'x' },
  '<UP>',
  "v:count == 0 ? 'gk' : 'k'",
  { desc = 'Up ignore wrapped', expr = true, silent = true }
)
-- swap g_ and $
...
map(
  ...
  '<LEADER>wo',
  '<C-W>o',
  { desc = 'Close all other windows', remap = true }
)
...
map(
  ...
  '<CR>',
  'viw<C-1>',
  { desc = 'Init Increment selection', remap = true }
)
