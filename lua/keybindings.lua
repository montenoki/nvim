...
map('n', keymaps.window.goto_left, '<C-w>h', { desc = 'Go to left window', remap = true })
map('n', keymaps.window.goto_lower, '<C-w>j', { desc = 'Go to lower window', remap = true })
map('n', keymaps.window.goto_upper, '<C-w>k', { desc = 'Go to upper window', remap = true })
map('n', keymaps.window.goto_right, '<C-w>l', { desc = 'Go to right window', remap = true })
...
map('n', keymaps.diagnostic.next_diag, diagnostic_goto(true), { desc = 'Next Diagnostic' })
map('n', keymaps.diagnostic.prev_diag, diagnostic_goto(false), { desc = 'Prev Diagnostic' })
map('n', keymaps.diagnostic.next_error, diagnostic_goto(true, 'ERROR'), { desc = 'Next Error' })
map('n', keymaps.diagnostic.prev_error, diagnostic_goto(false, 'ERROR'), { desc = 'Prev Error' })
map('n', keymaps.diagnostic.next_warn, diagnostic_goto(true, 'WARN'), { desc = 'Next Warning' })
map('n', keymaps.diagnostic.prev_warn, diagnostic_goto(false, 'WARN'), { desc = 'Prev Warning' })

-- toggle options
-- stylua: ignore start
map("n", keymaps.toggle.spelling, function() lazyvim.toggle("spell") end, { desc = "Toggle Spelling" })
map("n", keymaps.toggle.line_numbers, function() lazyvim.toggle.number() end, { desc = "Toggle Line Numbers" })
map("n", keymaps.toggle.relative_numbers, function() lazyvim.toggle("relativenumber") end, { desc = "Toggle Relative Line Numbers" })
map("n", keymaps.toggle.diagnostic, function() lazyvim.toggle.diagnostics() end, { desc = "Toggle Diagnostics" })
map("n", keymaps.toggle.wrap, function() toggle.wrap() end, { desc = "Toggle wrap" })

local conceallevel = vim.o.conceallevel > 0 and vim.o.conceallevel or 3
map("n", keymaps.toggle.conceal, function() lazyvim.toggle("conceallevel", false, { 0, conceallevel }) end, { desc = "Toggle Conceal" })

if vim.lsp.buf.inlay_hint or vim.lsp.inlay_hint then
  map("n", keymaps.toggle.inlay_hints, function() lazyvim.toggle.inlay_hints() end, { desc = "Toggle Inlay Hints" })
end

map("n", keymaps.toggle.treesitter, function() toggle.treesitter() end, { desc = "Toggle Treesitter Highlight" })
