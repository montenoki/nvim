...
按当前配置整理，包含直接添加和主动启用 LazyVim Extra 的功能插件；不包含依赖、仅修改配置的默认插件及未启用项。

| 名称 | 分类 | 用途 |
| --- | --- | --- |
| avante.nvim | AI 编程 | 在编辑器内与 AI 对话、生成和修改代码，当前接入 OpenRouter。 |
| jieba.vim | 中文编辑 | 在 Markdown 中按中文词语移动光标和选择文本。 |
| pangu.vim | 中文排版 | 整理中文与英文、数字之间的空格。 |
| im-select.nvim | 输入法 | 随编辑模式切换输入法。 |
| leap.nvim | 光标跳转 | 按字符定位目标，支持跨窗口跳转。 |
| flit.nvim | 光标跳转 | 增强 f、F、t、T 字符查找操作。 |
...
| venv-selector.nvim | Python | 查找和切换 Python 虚拟环境。 |
