local ok, err = xpcall(function()
    ...
    local outer =
        ls.session.current_nodes[vim.api.nvim_get_current_buf()].parent
    local placeholder =
        ls.session.current_nodes[vim.api.nvim_get_current_buf()].mark:pos_begin_raw()
    vim.api.nvim_win_set_cursor(0, { placeholder[1] + 1, placeholder[2] })
    ...
    assert(
        ls.session.current_nodes[vim.api.nvim_get_current_buf()]
            == inner.insert_nodes[2],
        "inner denominator"
    )
    config.snippets.jump(-1)
    assert(
        ls.session.current_nodes[vim.api.nvim_get_current_buf()]
            == inner.insert_nodes[1],
        "inner backward"
    )
    config.snippets.jump(1)
    config.snippets.jump(1)
